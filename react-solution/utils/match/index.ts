 ...
 * @returns {boolean}
 */
export function match(value, condition = {}, data = {}, separator = '.', errors) {
  let result = true;
  ...
  for (const key of keys) {
    if (condition[key] !== flatValue[key]) {
      ...
      let arrayEq = false;
      if (Array.isArray(condition[key]) && Array.isArray(flatValue[key]) && condition[key].length === flatValue[key].length) {
