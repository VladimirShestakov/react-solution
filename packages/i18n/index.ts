...
export class I18n {
   ...
   * Пока грузится словарь будет возвращаться option.fall или ключ перевода (key).
   * В режиме suspense грузится словарь будет выбрасываться исключением с промисом.
  ...
  translateSync = (key: I18nPath, options?: I18nTranslateOptions): string => {
    ...
    if (realLocale !== '*') {
      ...
      if (this.waiting.isWaiting(waitingKey) && options?.suspense) {
  ...
  async load(locale: string, namespace: string): Promise<void> {
    if (this.depends.dictionary[locale]?.[namespace]) {
      ...
      const translation = (typeof dic === 'function') ? (await dic()).default as I18nTranslation : dic;
