...
export class CacheStore implements ICacheStore {
  ...
  generateKey(values: (string | number | boolean)[]): string {
    if (values.length === 0) throw new Error('Need some values for generateKey');
  ...
  async isValid(key: string, options: TCacheValidateOptions): Promise<boolean> {
    ...
    if (options['if-modified-since']) {
      ...
      const sinceTime = Math.floor(new Date(options['if-modified-since']).getTime() / 1000);
   ...
   */
  async update(key: string, html: string, status: number = 200, maxAge: number, location?: string) {
    const data =
      status === 404 || location ? '' : this.config.compress ? await gzip.gzip(html) : html;
    const cache: TCache = {
      key,
      time: Date.now(),
      waiting: false,
